# StudySync Project Roadmap

## Project Description
StudySync is a Python-based application that helps students track assignments, due dates, and study sessions. Users can input tasks, view upcoming work, and stay organized.

## Target Users
College students who want a simple way to manage homework and deadlines.

## Features (Planned)
- Add assignments
- View assignments
- Mark assignments as complete
- Store assignments in a file
- Simple menu system

## Sprint 1 Tasks
- [x] Create GitHub repository
- [x] Create README.md
- [x] Write project idea
- [ ] Find related Python project on GitHub
- [ ] Test existing code and document findings

## Tools & Technologies
- Python
- File handling
- GitHub

## Stretch Goals
- Add due dates
- Add reminders
- Improve user interface
## Related Project Research
https://github.com/marcosvbras/todo-list-python

This project is similar because it allows users to create and manage tasks. It uses Python to build a simple task tracking system, which relates to my StudySync project.
